(() => {
    ...
    function menuInit(fullHeight = true) {
        ...
        if (iconMenu) {
            ...
            function toggleMenu(open = true) {
                ...
                } else if (open) document.documentElement.classList.add("menu-open");
            }
            function focusCatch(e) {
                if (e.which === 9) {
                    const focusEl = [ "a[href]", 'input:not([disabled]):not([type="hidden"]):not([aria-hidden])', "button:not([disabled]):not([aria-hidden])", "select:not([disabled]):not([aria-hidden])", "textarea:not([disabled]):not([aria-hidden])", "area[href]", "iframe", "object", "embed", "[contenteditable]", '[tabindex]:not([tabindex^="-"])' ];
                    const focusable = document.querySelector(".menu").querySelectorAll(focusEl);
    ...
    function fontSizeListener(func, params = false) {
        document.documentElement.addEventListener("transitionend", (e => {
            if (e.target === document.documentElement && e.propertyName === "font-size") params ? func(...params) : func();
        }));
    }
    ...
    const slideToggle = (target, duration = 500) => {
        if (target.hidden) return slideDown(target, duration); else return slideUp(target, duration);
    };
    ...
    function dataMediaQueries(array, dataSetValue) {
        ...
        if (media) {
            ...
            media.forEach((item => {
                ...
                const breakpoints = initBreakpoints();
                if (paramsArray[0] in breakpoints) breakpoint.value = convertTo("em", breakpoints[paramsArray[0]]); else breakpoint.value = convertTo("em", paramsArray[0]);
                breakpoint.type = paramsArray[1] ? paramsArray[1].trim() : "max";
                breakpoint.item = item;
                breakpointsArray.push(breakpoint);
            }));
            let mdQueries = breakpointsArray.map((function(item) {
                return "(" + item.type + "-width: " + item.value + ")," + item.value + "," + item.type;
            }));
    ...
    function tabs() {
        ...
        if (tabs.length > 0) {
            const hash = getHash();
            document.addEventListener("click", setTabsAction);
            ...
            if (mdQueriesArray && mdQueriesArray.length) mdQueriesArray.forEach((mdQueriesItem => {
                ...
                fontSizeListener(setTitlePosition, [ mdQueriesItem.itemsArray, mdQueriesItem.matchMedia ]);
                setTitlePosition(mdQueriesItem.itemsArray, mdQueriesItem.matchMedia);
            }));
        }
        ...
        function setTabsStatus(tabsBlock) {
            ...
            function isTabsAnimate(tabsBlock) {
                if (tabsBlock.hasAttribute("data-tabs-animate")) return tabsBlock.dataset.tabsAnimate > 0 ? tabsBlock.dataset.tabsAnimate - 20 / 2 : 140;
            }
            ...
            if (tabsContent.length > 0) {
                ...
                tabsContent.forEach(((tabsContentItem, index) => {
                    if (tabsTitles[index].classList.contains("_tab-active")) {
                        if (tabsBlockAnimate) tabsContentAnimate(tabsContentItem, tabsBlockAnimate); else tabsContentItem.hidden = false;
                        if (isHash && !tabsContentItem.closest(".popup")) setHash(`tab-${tabsBlockIndex}-${index}`);
                    } else if (tabsBlockAnimate) tabsContentAnimate(tabsContentItem, tabsBlockAnimate, false); else tabsContentItem.hidden = true;
                }));
            }
        }
        ...
        function tabsContentAnimate(tabsContentItem, tabsBlockAnimate, open = true) {
            tabsContentItem.style.transition = `opacity ${tabsBlockAnimate}ms ease`;
            if (open) setTimeout((() => {
                tabsContentItem.style.opacity = "0";
                tabsContentItem.hidden = false;
                setTimeout((() => {
                    tabsContentItem.style.opacity = "1";
                }), 20);
            }), tabsBlockAnimate); else {
                tabsContentItem.style.opacity = "1";
                tabsContentItem.style.opacity = "0";
                setTimeout((() => {
    ...
    if (document.querySelectorAll("[data-prlx-parent]")) new Parallax(document.querySelectorAll("[data-prlx-parent]"));
    class ScrollWatcher {
        constructor(props) {
            let defaultConfig = {
                logging: true
        ...
        scrollWatcherIntersecting(entry, observer, targetElement) {
            ...
            if (targetElement.dataset.watch === "top") if (entry.boundingClientRect.y < 0) {
        ...
        scrollWatcherCallback(entry, observer) {
            ...
            this.scrollWatcherIntersecting(entry, observer, targetElement);
    ...
    class DynamicAdapt {
        constructor(type) {
            this.type = type;
            this.breakpoints = initBreakpoints();
        }
        init() {
            this.оbjects = [];
            ...
            this.nodes.forEach((node => {
                ...
                if (dataArray[1] in this.breakpoints) оbject.breakpoint = this.breakpoints[dataArray[1]]; else оbject.breakpoint = dataArray[1] ? dataArray[1].trim() : this.breakpoints.lg;
                оbject.place = dataArray[2] ? dataArray[2].trim() : "last";
                оbject.index = this.indexInParent(оbject.parent, оbject.element);
                this.оbjects.push(оbject);
            }));
            this.arraySort(this.оbjects);
            this.mediaQueries = this.оbjects.map((({breakpoint}) => `(${this.type}-width: ${convertTo("em", breakpoint)}),${breakpoint}`)).filter(((item, index, self) => self.indexOf(item) === index));
            this.mediaQueries.forEach((media => {
                ...
                this.mediaHandler(matchMedia, оbjectsFilter);
                document.documentElement.addEventListener("transitionend", (e => {
                    if (e.target === document.documentElement) if (e.propertyName === "font-size") this.mediaHandler(matchMedia, оbjectsFilter);
                }));
            }));
        }
